...
settings_file = "modhis_settings.txt"
settings = {}
...
### Telescope Setup
tmt = telescope.TMT(path=settings['path'])
if 'airmass' in settings: tmt.airmass = float(settings['airmass'])
if 'water_vapor' in settings: tmt.water_vapor = float(settings['water_vapor'])
if 'seeing' in settings:
    tmt.seeing = float(settings['seeing'])
else: 
    tmt.seeing = tmt.median_seeing

### Instrument Setup
modhis = instrument.modhis(telescope=tmt)
modhis.set_current_filter(settings['filter'])
...
modhis.set_observing_mode(float(settings['itime']),int(settings['nint']),
...
host_spectrum = spectrum.get_stellar_spectrum(host_properties,wavelengths,
                                              modhis.current_R,model="Phoenix",
...
obj_user_params = (settings['path'],settings['companionfilter'],float(settings['mag']),filters,modhis.current_filter)
...
for modhis_filter in modhis.filters:
    ...
    wavelengths = modhis.get_wavelength_range()
    modhis.set_observing_mode(3600,1,modhis_filter, wavelengths) 
    
    host_user_params = (settings['path'],settings['starfilter'],float(settings['starmag']),filters,modhis.current_filter) 
    ...
    full_speckle_noise.append(modhis.get_speckle_noise(0.4*u.arcsecond,host_properties["StarAOmag"],modhis.current_filter,
