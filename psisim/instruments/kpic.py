...
class kpic_phaseI(Instrument):
    ...
    def get_inst_throughput(self,wvs,planet_flag=False,planet_sep=None):
        ...
        KAO_th = self.get_AO_throughput(wvs)
...
# th_ao = {"CFHT-Y":0.60,"TwoMASS-J":0.63,"TwoMASS-H":0.66,"TwoMASS-K":0.633}.get(self.current_filter) #K-band from Nem's Feb 2020 report
        if self.mode == 'vfn':
            ...
            KPIC1FIU_th = self.get_FIU_throughput(wvs)
            fiber_th = self.get_fiber_throughput(wvs)
        ...
        nirspec_th = self.get_spec_throughput(wvs)
        feu_th = self.get_feu_throughput(wvs)

        #TODO: figure out if SR is needed for VFN (thinking it's not)
        strehl = self.compute_SR(wvs)
        if self.mode == 'vfn':
            strehl = np.ones(strehl.shape)

        diff_keck = 0.60

        th_inst = KAO_th * KPIC1FIU_th * fiber_th * feu_th * nirspec_th * strehl * diff_keck*th_planet
    ...
    def get_spec_throughput(self, wvs):
        ...
        if self.current_filter == "TwoMASS-K":
            wave = wvs.to(u.um).value
            nirspec_th = np.ones(len(wvs)) * 0.2 # Measured by G. Doppman (K-band value)
            wvs_order=np.load(datadir + '/throughput/wvs_order.npy')
            blaze_order=np.load(datadir + '/throughput/blaze_order.npy')
            temp=np.zeros(len(wvs))
            for i in range(9):
                f=interp1d(wvs_order[i,:],blaze_order[i,:],fill_value=0,bounds_error=False, kind='zero')
                temp = temp + nirspec_th*f(wave)
            th_spec=temp
        else:
    ...
    def get_FIU_throughput(self,wvs):
        '''
        Returns the FIU throughput at each wavelength
        '''
        wave = wvs.to(u.um).value

        protected_au_data = np.genfromtxt(datadir + '/throughput/protected_au.csv', delimiter=',', skip_header=1)
        FOAP1_th = np.interp(wave,protected_au_data[:,0],protected_au_data[:,1])
        FM1_th = FOAP1_th
        PWFSDICH_th = 0.94
        FOAP2_th = FOAP1_th
        FOAP3_th = FOAP1_th
        ...
        TRACDICH_th = 0.94
        FOAP6_th = FOAP1_th
        KPIC1FIU_th = FOAP1_th * FM1_th * PWFSDICH_th * FOAP2_th * FOAP3_th * FOAP4_th * FOAP5_th * FM2_th * TRACDICH_th * FOAP6_th
    ...
    def get_AO_throughput(self,wvs):
        '''
        Returns the Keck AO throughput at each wavelength
        '''
        wave = wvs.to(u.um).value
        ...
        FSM1_th = TTM_th
        FSM2_th = TTM_th
        KAO_th = K1_th * K2_th * K3_th * TTM_th * OAP1_th * DM_th * OAP2_th * SHWFSDICH_th * PICKOFF_th * FSM1_th * FSM2_th

        return KAO_th

    def get_fiber_throughput(self,wvs):
        '''
        Returns the fiber throughput at each wavelength
        '''
        wave = wvs.to(u.um).value

        kpic12_klfiber_data = np.genfromtxt(datadir + '/throughput/kpic12_klfiber.csv', delimiter=',', skip_header=1)
        fiberin_th = 0.98
        fiberprop_th = np.interp(wave, kpic12_klfiber_data[:,0],kpic12_klfiber_data[:,1])
        fiberout_th = 0.98
        fiber_th = fiberin_th * fiberprop_th * fiberout_th

        return fiber_th

    def get_feu_throughput(self,wvs):
        '''
        Returns the FEU throughput at each wavelength
        '''
        wave = wvs.to(u.um).value

        feu_th = np.ones(len(wave))
        feu_th[:] = 0.73 #measured
    ...
    def get_instrument_background(self,wvs,solidangle):
        ...
        fei_therm *= 1-KAO_th * KPIC1FIU_th
        fei_therm *= fiber_th*feu_th
        ...
        fiber_therm *= feu_th*self.get_spec_throughput(wvs)

        hardcoded_background_scaling = 40#1#40
        bb_lam_feu = BlackBody(self.temperature_feu, scale=1.0 * u.erg / (u.cm ** 2 * u.AA * u.s * u.sr))
        feu_therm = bb_lam_feu(wvs)
        feu_therm *= solidangle* self.telescope.collecting_area
        feu_therm = feu_therm.to(u.ph / (u.um * u.s), equivalencies=u.spectral_density(wvs))
        feu_therm *= (1-feu_th)*hardcoded_background_scaling
        feu_therm *= self.get_spec_throughput(wvs)
        ...
        spec_therm = spec_therm.to(u.ph / (u.micron * u.s * u.cm ** 2),
                                   equivalencies=u.spectral_density(wvs)) * self.telescope.collecting_area
        spec_therm *= 1-self.get_spec_throughput(wvs)

        return fei_therm + fiber_therm +feu_therm+ spec_therm
...
class kpic_phaseII(Instrument):
    ...
    def __init__(self, telescope=None, use_adc=True):
        ...
        # The main instrument properties - static
        self.read_noise = 10 *u.electron # * u.photon#e-/pix/fr
        self.dark_current = 1*u.electron/u.s#0.67*u.electron/u.s #electrons/s/pix
        self.det_welldepth = 1e5 *u.photon
        self.det_linearity = 0.66*self.det_welldepth
        self.qe = 0.95 * u.electron/u.ph
    ...
    def get_inst_throughput(self,wvs,planet_flag=False,planet_sep=None):
        ...
        strehl = self.compute_SR(wvs)
        if self.mode == 'vfn':
            strehl = np.ones(SR.shape)

        nirspec_th = self.get_spec_throughput(wvs)

        diff_keck = 0.655 #theoretical coupling efficiency due to not perfect gaussian PSF (e.g., diffraction spikes etc.)
        PIAA_boost = 1.2
        coupling_efficiency = strehl * diff_keck #* PIAA_boost

        th_inst = KAO_th * KPIC1FIU_th * fiber_th * feu_th * nirspec_th * coupling_efficiency * th_planet
        #sky_trans_interp * tel_th *
        # KAO_th * KPIC1FIU_th * fiber_th * feu_th * nirspec_th * coupling_efficiency
    ...
    #
    #     # TODO: do we want to use the throughput w/ or w/o the planet losses?
    #     return (1-self.get_inst_throughput(wvs,planet_flag=False) / coupling_efficiency)
    ...
    def get_spec_throughput(self, wvs):
        ...
        if self.current_filter == "TwoMASS-K":
            wave = wvs.to(u.um).value
            nirspec_th = np.ones(len(wvs)) * 0.2 * 1.5 # Measured by G. Doppman (K-band value), accounting for new K-band filter (50% boost)
            wvs_order=np.load(datadir + '/throughput/wvs_order.npy')
            blaze_order=np.load(datadir + '/throughput/blaze_order.npy')
            temp=np.zeros(len(wvs))
            for i in range(9):
                ...
                temp = temp + nirspec_th*f(wave)
            th_spec=temp
            # th_spec = 0.2*1.5*si.interp1d(self.nirspec_th_data[:,0], self.nirspec_th_data[:,1],bounds_error=False,fill_value=0)(wvs)
        else:
            th_spec = {"CFHT-Y":0.5,"TwoMASS-J":0.5,"TwoMASS-H":0.5,"TwoMASS-K":0.2*1.5}.get(self.current_filter,0.5)
            th_spec = th_spec*np.ones(np.shape(wvs))
    ...
    def get_FIU_throughput(self,wvs):
        ...
        wave = wvs.to(u.um).value
        protected_au_data = np.genfromtxt(datadir + '/throughput/protected_au.csv', delimiter=',', skip_header=1)
        kpic2_dmandwindow_data = np.genfromtxt(datadir + '/throughput/kpic2_dmandwindow.csv', delimiter=',', skip_header=1)

        FOAP1_th = np.interp(wave,protected_au_data[:,0],protected_au_data[:,1])
        DM1_th = np.interp(wave,protected_au_data[:,0],protected_au_data[:,1])#np.interp(wave,kpic2_dmandwindow_data[:,0],kpic2_dmandwindow_data[:,1])
        PWFSDICH_th = 0.94
        FOAP2_th = FOAP1_th
        FOAP3_th = FOAP1_th
        FOAP4_th = FOAP1_th
        ...
        KPIC1FIU_th = FOAP1_th * DM1_th * PWFSDICH_th * FOAP2_th * FOAP3_th * FOAP4_th * FOAP5_th * FM2_th * TRACDICH_th * injectionlens_th
        return KPIC1FIU_th
    ...
    def get_feu_throughput(self,wvs):
        ...
        extractionlens_th = 0.955
        FEU_M1_th = np.interp(wave,protected_au_data[:,0],protected_au_data[:,1])
        FEU_M2_th = FEU_M1_th
        FEU_M3_th = FEU_M1_th
        FEU_M4_th = FEU_M1_th
        feu_th = extractionlens_th * FEU_M1_th**4
        return feu_th
