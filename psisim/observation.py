...
####
def simulate_observation_singletarget(telescope, instrument, wavelengths, model_spectrum,
                                inject_noise = False, sky_on = False, post_processing_gain = 1, return_noise_components = False,
                                apply_lsf = False, integrate_delta_wv = False):
    '''
    A simplified version of simulate_observation for single target observations
    
    Inputs
    ------
    telescope : a psisim telescope object
    instrument : a psisim instrument object
    model_spectrum : a psisim spectrum object
        expected units are photons/cm^2/s/Angstrom
    wavelengths : array, length N
    ...
    Returns
    -------
    detector_spectrum : array, units of e-
        spectrum that gets read off by the detector
    total_noise : arraylike, units of e-
        photon + read noise (not considering speckle noise)
    detector_thermal_flux : arraylike, units of e-
    -------
    ...
    Based on the code from Connor Vancil
    
    '''
    # Multiply spectrum by telescope collecting area
    instrument_spectrum = model_spectrum * telescope.collecting_area.to(u.cm**2)

    # Multiply the spectrum by atmospheric transmission if requested
    if sky_on:
    ...
    instrument_spectrum *= telescope.get_telescope_throughput(wavelengths)
    ...
    thermal_sky *= telescope.get_telescope_throughput(wavelengths) #Multiply by telescope throughput
    thermal_sky *= instrument.get_inst_throughput(wavelengths) #Multiply by instrument throughput
    
    ## total throughput of instrument
    tot_th_instrument = instrument.get_inst_throughput(wavelengths) * telescope.get_telescope_throughput(wavelengths)
    
    # Get Telescope thermal background in photons/s/Angstrom
    thermal_telescope = telescope.get_thermal_emission(wavelengths)
    ...
    # Ignoring speckle noise for single target observations - old note in PSISIM code says
    read_noise = np.ones(np.shape(wavelengths.value)) * np.sqrt(instrument.n_exposures) * instrument.read_noise * instrument.spatial_sampling
    ...
    if return_noise_components:
        return detector_spectrum, total_noise, detector_thermal_flux, np.array([photon_noise, read_noise]),tot_th_instrument
    else:
        return detector_spectrum, total_noise, detector_thermal_flux,tot_th_instrument
...
def simulate_observation(telescope,instrument,planet_table_entry,planet_spectrum,wvs,spectrum_R,
    ...
    #Multiply by telescope throughput
    stellar_spectrum *= telescope.get_telescope_throughput(wvs)
    ...
    thermal_sky *= telescope.collecting_area.to(u.cm**2) #Multiply by collecting area - units of photons/s/Angstom
    thermal_sky *= telescope.get_telescope_throughput(wvs) #Multiply by telescope throughput
    thermal_sky *= instrument.get_inst_throughput(wvs) #Multiply by instrument throughput
    
    #Get Telescope thermal background in photons/s/Angstrom
    thermal_telescope = telescope.get_thermal_emission(wvs)
    thermal_telescope *= telescope.collecting_area.to(u.cm**2) #Now in units of photons/s/Angstrom
...
def simulate_bg(telescope,instrument,planet_table_entry,planet_spectrum,wvs,spectrum_R,verbose=False,post_processing_gain = 1,return_noise_components=False,stellar_spec=None,no_speckle_noise=False,plot=False, sky_on=True):
    '''
    A function that simulates an observation
    ...
    thermal_sky *= instrument.qe
    thermal_telescope *= instrument.qe
    thermal_inst *= instrument.qe

    #Downsample to instrument wavelength sampling
    intermediate_thermal_sky = si.interp1d(wvs, thermal_sky,fill_value="extrapolate",bounds_error=False)
    intermediate_thermal_telescope = si.interp1d(wvs, thermal_telescope,fill_value="extrapolate",bounds_error=False)
    intermediate_thermal_inst = si.interp1d(wvs, thermal_inst,fill_value="extrapolate",bounds_error=False)

    
    detector_sky = 1e4*u.AA/u.micron*intermediate_thermal_sky(instrument.current_wvs)*instrument.current_dwvs*thermal_sky.unit
    detector_telescope = 1e4*u.AA/u.micron*intermediate_thermal_telescope(instrument.current_wvs)*instrument.current_dwvs*thermal_telescope.unit
    detector_inst = 1e4*u.AA/u.micron*intermediate_thermal_inst(instrument.current_wvs)*instrument.current_dwvs*thermal_inst.unit

    #Multiply by the exposure time
    detector_sky *= instrument.exposure_time #The detector spectrum is now in e-
    detector_telescope *= instrument.exposure_time #The detector spectrum is now in e-
    detector_inst *= instrument.exposure_time

    #Multiply by the number of exposures
    detector_sky *= instrument.n_exposures
    detector_telescope *= instrument.n_exposures
    detector_inst *= instrument.n_exposures
    ...
    return detector_sky, detector_telescope, detector_inst,dark_current
