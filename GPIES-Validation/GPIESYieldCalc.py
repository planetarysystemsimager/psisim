...
if n_planets > 0:   
    ...
    for planet in planet_table:
        ...
        planet_spectrum = spectrum.get_stellar_spectrum(planet,[wvs.value]*wvs.unit,R,model='COND+BTSettle_Broadband',verbose=False,user_params = None,doppler_shift=False,broaden=False,delta_wv=None).spectrum
        planet_spectra.append(planet_spectrum)

        #get stellar spectra
        stellar_spectrum = spectrum.get_stellar_spectrum(planet,[wvs.value]*wvs.unit,R,model='Speclite_H_mag').spectrum
        stellar_spectra.append(stellar_spectrum)
    ...
    sim_F_lambda, sim_F_lambda_errs, sim_F_lambda_stellar, noise_components = observation.simulate_observation_set(gemini,gpi, planet_table, planet_spectra, [wvs.value]*wvs.unit, R, inject_noise=False,post_processing_gain=post_processing_gain,return_noise_components=True,stellar_spec=stellar_spectra)
